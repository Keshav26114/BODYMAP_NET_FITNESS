...
def list_users_full():
    """
    Return every registered user with full profile detail (everything
    except password_hash), plus how many test runs each has logged, for the
    admin "Show More" overview page. Sorted alphabetically by name.
    """
    conn = get_connection()
    try:
        rows = conn.execute(
            "SELECT u.unique_id, u.name, u.age, u.gender, u.body_goal, u.created_at, "
            "COUNT(t.run_id) AS run_count "
            "FROM users u LEFT JOIN test_runs t ON t.unique_id = u.unique_id "
