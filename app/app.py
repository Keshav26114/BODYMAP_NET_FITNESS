...
def history():
    ...
    if is_user:
        # Users always see their own weekly records directly — no ID
        # lookup/search needed, since there's nothing to pick between.
        user = db.get_user(own_id)
        if user is None:
            return render_template("history.html", error="Your account could not be found.",
                                   unique_id=own_id, users=users_list)
        runs = db.get_runs_for_user(own_id)
        weekly_streaks = streaks.compute_weekly_streaks(user, db.get_weeks_for_user(own_id))
        return render_template("history.html", user=user, runs=runs, unique_id=own_id,
...
@app.route("/settings/admin/all-users")
@admin_required
def admin_all_users():
    """Admin-only overview of every account, with more detail than fits on
    the Settings page's compact grid (age, gender, goal, join date, number
    of logged weeks) — useful for browsing/searching across many profiles."""
    return render_template("admin_all_users.html", all_users_full=db.list_users_full())
